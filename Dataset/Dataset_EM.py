...
class MyDataset_EM_format_2(Dataset):
    def __init__(self,input_folder):
        ...
        self.input_folder=input_folder
        self.data={}
        self.__get_files(self.input_folder)

    def __get_files(self,path) : 
         
         list_files=os.listdir(os.path.join(path,"images"))
         ...
         for file in list_files : 
              if file.endswith(".tif") : 
                ...
                image=np.array(image)
                #mask=np.full(shape=(*image.shape,3), fill_value=cat)
                mask=Image.open(os.path.join(path,"labels","{}.png".format(file.split(".tif")[0])))
...
if __name__=="__main__": 
    ...
    dataset=MyDataset_EM_format_2(input_folder="/home/adrienb/Documents/Data/DataSet_256_256_classif_128_128_format_2/raw")
